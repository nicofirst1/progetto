...
def cluster_dict_syn(word_for_cluster=100):
    ...
    carica il modello word2vec di google (opportunamente ottimizzato)
    su cui è stato effettuato un train con 3 milioni di parole;
    divide le parole in cluster per similitudine semantica,
    tramite un analisi a k-means (il numero di parole per cluster è dettato dal parametro facoltativo);
    ...
    # devo decidere le dimensioni del km,
    # ovvero quante parole sono presenti in un cluster, per ora mi tengo sulle 100 parole
